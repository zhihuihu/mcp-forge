import type { JsonValue, SqlserverExecutionResult, SqlStatementKind } from './types.js';

interface ResultPayload {
  schema_version: 1;
  statementType: SqlStatementKind;
  columns: SqlserverExecutionResult['fields'];
  rows: JsonValue[];
  rowCount: number;
  returnedRows: number;
  truncated: boolean;
...
export function formatResult(
  result: SqlserverExecutionResult,
  statementType: SqlStatementKind,
  maxRows: number,
  ...
  const basePayload = {
    ...
    ...(result.returnValue !== undefined ? { returnValue: normalizeValue(result.returnValue) } : {}),
  };
