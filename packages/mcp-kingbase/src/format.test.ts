...
import type { KingbaseExecutionResult } from './types.js';

describe('formatResult', () => {
  const sampleResult: KingbaseExecutionResult = {
    rows: [
  ...
  it('uses binary search to quickly truncate results exceeding maxResultBytes', () => {
    ...
    const largeResult: KingbaseExecutionResult = {
      rows: manyRows,
  ...
  it('handles bigint and buffer values correctly', () => {
    const specialResult: KingbaseExecutionResult = {
      rows: [{ big_id: BigInt('9007199254740993'), bytea_data: Buffer.from('kingbase-raw') }],
      fields: [{ name: 'big_id' }, { name: 'bytea_data' }],
    };
    ...
    expect(parsed.rows[0].bytea_data).toBe(Buffer.from('kingbase-raw').toString('base64'));
  });
});
