import type { KingbaseMode, SqlClassification, SqlStatementKind } from './types.js';
...
function tokenize(sql: string): SqlToken[] {
  ...
  while (index < sql.length) {
    ...
    // Line comments: -- (standard / PG / Oracle) or # (MySQL mode)
    ...
    // Block comment: /* ... */ (supports nested block comments like PG)
    if (character === '/' && next === '*') {
      index += 2;
      let depth = 1;
    ...
    // Single quotes (string), Double quotes or Backticks (identifiers in PG / Oracle / MySQL modes)
    if (character === "'" || character === '"' || character === '`') {
      ...
      tokens.push({ kind: quote === "'" ? 'string' : 'word', value: '<quoted>' });
      continue;
    }
...
function findMainStatementKeywordInWith(tokens: readonly SqlToken[]): string | undefined {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (depth === 0 && seenParenthesis && token.kind === 'word') {
      ...
      if (['SELECT', 'TABLE', 'VALUES', 'INSERT', 'UPDATE', 'DELETE', 'MERGE'].includes(upper)) {
        return upper;
      }
    }
  }
...
function classify(tokens: readonly SqlToken[]): { kind: SqlStatementKind; firstKeyword: string } {
  ...
  // Read-only queries (including SHOW, DESC, DESCRIBE for MySQL / Oracle modes)
  if (
    value === 'SELECT' ||
    value === 'TABLE' ||
    value === 'VALUES' ||
    ...
    // In PostgreSQL / KingbaseES, EXPLAIN ANALYZE actually executes the statement!
    if (
      value === 'EXPLAIN' &&
      valueWords.includes('ANALYZE') &&
      containsAny(valueWords, ['INSERT', 'UPDATE', 'DELETE', 'MERGE', 'REPLACE'])
    ) {
      return { kind: 'dml', firstKeyword: value };
    }
    return { kind: 'read', firstKeyword: value };
  ...
  // DML (including REPLACE for MySQL mode, MERGE for Oracle mode)
  if (
    value === 'INSERT' ||
    value === 'UPDATE' ||
    value === 'DELETE' ||
...
export function enforceSqlPolicy(sql: string, mode: KingbaseMode): SqlClassification {
