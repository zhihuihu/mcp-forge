...
function annotations(config: PostgresConfig) {
...
const queryInput = {
  sql: z.string().min(1).describe('One PostgreSQL statement. Multiple statements are not allowed.'),
  params: z
    .array(z.unknown())
    .optional()
...
export function createServer(config: PostgresConfig, client: PgClient): McpServer {
  const server = new McpServer({ name: 'mcp-postgres', version: packageVersion });

  server.registerTool(
    'pg_query',
    {
      title: 'Execute a PostgreSQL statement',
      description:
        'Execute one PostgreSQL statement under the configured readonly, write, or admin policy. Supports $1, $2, ... placeholders.',
    ...
    async (input) => {
      try {
        ...
        return failure('PostgreSQL query failed', error);
      }
    },
  );
  ...
  server.registerTool(
    'pg_get_server_info',
    {
      title: 'Get PostgreSQL server information',
      description: 'Return the PostgreSQL version, current user, database, and schema search path.',
    ...
    async () => {
      try {
        ...
        return failure('Unable to get PostgreSQL server information', error);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async () => {
      try {
        ...
        return failure('Unable to list PostgreSQL databases', error);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async () => {
      try {
        ...
        return failure('Unable to list PostgreSQL schemas', error);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async (input) => {
      try {
        ...
        return failure('Unable to list PostgreSQL tables', error);
      }
    },
  );
    ...
    async (input) => {
      try {
        ...
        if (columnRows.length === 0) {
          return failure(
            'Unable to describe PostgreSQL table',
        ...
        return failure('Unable to describe PostgreSQL table', error);
      }
    },
  );
